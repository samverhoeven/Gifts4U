		...
		foreach ( $fields as $field ) {
			...
			elseif ( $field == 'product_tag' ) { _e( 'Tags', 'color-filters' ); }
				...
				foreach ( ewd_uwcf_get_woocommerce_taxonomies() as $attribute_taxonomy ) {
					...
					echo $attribute_taxonomy->attribute_label;
				}
			}
							...
							<tr data-field="<?php echo $field; ?>">
								...
								<td>
									<div class="sap-admin-hide-radios">
										<input type='checkbox' name='<?php echo $field; ?>_displayed' value='1' <?php echo ( $this->field_displayed( $field ) ? 'checked' : '' ); ?> >
									</div>
									<label class="sap-admin-switch">
										<input type="checkbox" class="sap-admin-option-toggle" data-inputname="<?php echo $field; ?>_displayed" <?php echo ( $this->field_displayed( $field ) ? 'checked' : '' ); ?>>
								...
								<td>
									<?php if ( $this->filtering_possible( $field ) ) { ?>
										<div class="sap-admin-hide-radios">
											<input type='checkbox' name='<?php echo $field; ?>_enable_filtering' value='1' <?php echo ( $this->field_displayed( $field ) ? 'checked' : '' ); ?> >
										</div>
										<label class="sap-admin-switch">
											<input type="checkbox" class="sap-admin-option-toggle" data-inputname="<?php echo $field; ?>_enable_filtering" <?php echo ( $this->field_filtering_enabled( $field ) ? 'checked' : '' ); ?>>
											<span class="sap-admin-switch-slider round"></span>
										</label>
									<?php } ?>
								</td>
									...
									<?php if ( $this->filtering_possible( $field ) ) { ?>
										<select name='<?php echo $field; ?>_filter_type' class='ewd-uwcf-wc-format-filter-type'>
											<?php foreach ( $this->field_filtering_options( $field ) as $filtering_option ) { ?>
												<option value='<?php echo $filtering_option; ?>' <?php echo ( $this->field_filtering_type( $field ) == $filtering_option ? 'selected' : ''); ?>><?php echo $filtering_option ?></option>
											<?php } ?>
										</select>
									<?php } ?>
								</td>
	...
	public function get_field_name( $field ) { 
		...
		elseif ( $field == 'sizes' ) 		{ return __( 'Sizes', 'color-filters' ); }
			...
			foreach (ewd_uwcf_get_woocommerce_taxonomies() as $attribute_taxonomy ) {
				...
				if ( $attribute_taxonomy->attribute_name == $field ) { return $attribute_taxonomy->attribute_label; }
			}
		}
	}
