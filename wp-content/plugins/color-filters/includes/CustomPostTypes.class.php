...
class ewduwcfCustomPostTypes {
	...
	public function edit_color_field( $term ) { 
		...
		<tr class="form-field term-color-wrap cf-color-filters"> 
			...
			<td>
				...
				<input class="cf-color small-text" name="normal_fill" id="normal_fill_color" type="text" size="40" value="<?php echo ( strlen( $color ) <= 7 ? $color : '' ); ?>" /><br/>
				<label><?php _e("Or upload an image of the color pattern below:", 'color-filter'); ?></label>
				<input id="color_image" type="text" size="36" name="color_image" value="<?php echo ( strlen( $color ) > 7 ? $color : 'http://' ); ?>" />
				<input id="color_image_button" class="button" type="button" value="Upload Image" />
			</td>
		</tr>
	 ...
	 * @since 3.0.0
	 */
	public function update_size_data( $term_id, $taxonomy ) { 
	...
	/**
	 * Displays the content for the color color column of the color taxonomy
	 * @since 3.0.0
	 */
	...
	public function update_color_order() {
		...
		foreach ( $ids as $order => $term_id ) {
			...
			update_term_meta( intval( $term_id ), 'EWD_UWCF_Term_Order', $order );
		}
	}
