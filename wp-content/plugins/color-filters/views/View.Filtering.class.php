...
class ewduwcfViewFiltering extends ewduwcfView {
	...
	public function maybe_print_color_filtering() {
		...
		$this->filtering_args = array(
			'type'					=> 'color',
			'terms' 				=> $terms,
			'selected_values'		=> isset( $_GET['product_color'] ) ? explode( ',', $_GET['product_color'] ) : array(),
	...
	public function maybe_print_size_filtering() {
		...
		$this->filtering_args = array(
			'type'					=> 'size',
			'terms' 				=> $terms,
			'selected_values'		=> isset( $_GET['product_size'] ) ? explode( ',', $_GET['product_size'] ) : array(),
	...
	public function maybe_print_category_filtering() {
		...
		$this->filtering_args = array(
			'type'					=> 'category',
			'terms' 				=> $terms,
			'selected_values'		=> isset( $_GET['product_cat'] ) ? explode( ',', $_GET['product_cat'] ) : array(),
	...
	public function maybe_print_tag_filtering() {
		...
		$this->filtering_args = array(
			'type'					=> 'tag',
			'terms' 				=> $terms,
			'selected_values'		=> isset( $_GET['product_tag'] ) ? explode( ',', $_GET['product_tag'] ) : array(),
	...
	public function maybe_print_filtering_for_attribute( $attribute_taxonomy ) {
		...
		$this->filtering_args = array(
			...
			'terms' 				=> $terms,
			'selected_values'		=> isset( $_GET['pa_' . $attribute_taxonomy->attribute_name] ) ? explode( ',', $_GET['pa_' . $attribute_taxonomy->attribute_name] ) : array(),
	...
	public function get_color_style( $term ) {
		...
		$style = strpos($color, 'http' ) === false ? 'style="background: ' . $color . ';"'  : 'style="background:url(\'' . $color . '\'); background-size: cover;"';
	...
	public function get_max_wc_price() {
		...
		return $wpdb->get_var("SELECT MAX(CAST(meta_value as DECIMAL(12,2))) FROM $wpdb->postmeta INNER JOIN $wpdb->posts ON $wpdb->postmeta.post_id = $wpdb->posts.ID WHERE $wpdb->posts.post_type = 'product' AND $wpdb->postmeta.meta_key = '_price'");
	}
	...
	public function enqueue_assets() {
		...
		wp_localize_script( 'ewd-uwcf-js', 'ewd_uwcf_php_data', $filtering_data );
